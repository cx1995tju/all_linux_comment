				      ...
				      unsigned int rto_base)
{
	...
	linear_backoff_thresh = ilog2(TCP_RTO_MAX / rto_base); // 120s : 200ms = 9
	if (boundary <= linear_backoff_thresh)
		timeout = ((2 << boundary) - 1) * rto_base;
	else
		timeout = ((2 << linear_backoff_thresh) - 1) * rto_base + // (1024 - 1) * 200ms = 204.6s
			(boundary - linear_backoff_thresh) * TCP_RTO_MAX; // 6 * 120s = 720ss
