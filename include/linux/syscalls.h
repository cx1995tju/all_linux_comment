...
// 即将 m 应用于 __VA_ARGS__ 中的每一对相邻的参数
#define __MAP0(m,...)
...
// sname: syscall name
//
// 定义一个 metadata 结构，然后放置到特定的 section 中，本质就是注册了这个 metadata
#define SYSCALL_METADATA(sname, nb, ...)			\
	/* 将参数中的所有 type 拿出来，组成字符串数组 ["unsigned int", "const char __user", "size_t"] */		\
	static const char *types_##sname[] = {			\
		__MAP(nb,__SC_STR_TDECL,__VA_ARGS__)		\
	};							\
	static const char *args_##sname[] = {			\
	/* 所有参数拿出来组成字符串数组, ["fd", "buf", "count"] */		\
		__MAP(nb,__SC_STR_ADECL,__VA_ARGS__)		\
	};							\
	SYSCALL_TRACE_ENTER_EVENT(sname);			\
	SYSCALL_TRACE_EXIT_EVENT(sname);			\
	/* 结合前面定义的数组，函数，构成一个 syscall 的metadata 结构 */ \
	...
	/* XXX NOTE: 取前面 metadata 的指针，然后放置到一个 特殊的 seciton 中，本质就是注册了这个 syscall 了 */ \
	static struct syscall_metadata __used			\
 ...
 */
// 为什么这么复杂是因为：The other functions that are generated by the __SYSCALL_DEFINEx macro are need to protect from the CVE-2009-0029
// 通过 name 查找 `./arch/x86/entry/syscalls/syscall_64.tbl` 可以有唯一的系统调用号的
//
// 系统调用流程
// 入口: arch/x86/entry/entry_64.S:entry_SYSCALL_64()
// 通过系统调用号和 syscall_64.tbl 可以找到类似的 sys_write 的函数入口
// sys_wirte 就是 __se_sys_write
// 而 __se_sys_write 最主要的 body 就是 __do_sys_write
